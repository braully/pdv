...
public interface ParcelaRepository extends JpaRepository<Parcela, Long> {
    ...
    @Query(value = "select coalesce(sum(p.valor_restante), '0.00') from parcela p, receber r, pessoa pes where r.codigo = p.receber_codigo and  pes.codigo = r.pessoa_codigo "
            + "and pes.codigo = :codigo and p.quitado = :quitado order by p.codigo", nativeQuery = true)
